...
const moveTo = (item, spot) => (spot < 7 ? item.style.setProperty("grid-column", spot) : null);
...
// Click Handlers
const boostApollo = () => {     
  apollo_spot += Math.floor(Math.random() * 3) + 1;
  moveTo(apolloRocket, apollo_spot);
};

const boostNebula = () => {
  nebula_spot += Math.floor(Math.random() * 3) + 1;
  moveTo(nebulaRocket, nebula_spot);
};

const boostComet = () => {
  comet_spot += Math.floor(Math.random() * 3) + 1;
